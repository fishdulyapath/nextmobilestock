...
import 'package:mobilestock/features/cart/cart_read_detail_screen.dart';
import 'package:mobilestock/features/handheld/handheld_detail_screen.dart';
import 'package:mobilestock/features/handheld/handheld_form_screen.dart';
import 'package:mobilestock/features/handheld/handheld_merge_screen.dart';
import 'package:mobilestock/model/cart_model.dart';
import 'package:mobilestock/repository/webservice_repository.dart';

class HandheldListScreen extends StatefulWidget {
  const HandheldListScreen({super.key});

  @override
  State<HandheldListScreen> createState() => _HandheldListScreenState();
}
                    ...
                    child: _buildActionButton(
                      ...
                      onPressed: () {
                        final res = Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => HandheldFormScreen(cart: CartModel(docno: '')),
                          ),
                        );
                        res.then((value) {
                          getCartList();
  ...
  Widget _buildCartCard(CartModel cart, int index) {
    ...
    return Container(
      ...
      child: Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(16),
          onTap: showCheckbox && cart.status == 0
              ? () {
                  setState(() {
                          ...
                          _buildIconButton(
                            ...
                            onPressed: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => HandheldFormScreen(cart: cart),
                                ),
                              ).then((value) {
                                getCartList();
                              });
                        ...
                        _buildIconButton(
                          ...
                          onPressed: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => HandheldDetailScreen(cart: cart, ismerge: cart.ismerge),
                              ),
                            ).then((value) {
                              getCartList();
                            });
    ...
    await _webServiceRepository.getCartDetail(docno).then((value) {
      ...
      if (value.success) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => CartReadDetailScreen(
              cart: cart,
              ...
              Builder(
                builder: (context) {
                  ...
                  return Container(
                    ...
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(
                      color: isSameWarehouseLocation ? const Color(0xFF10B981).withOpacity(0.05) : const Color(0xFFF59E0B).withOpacity(0.1),
                      borderRadius: BorderRadius.circular(10),
                      border: !isSameWarehouseLocation ? Border.all(color: const Color(0xFFF59E0B).withOpacity(0.5)) : null,
                    ),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        const SizedBox(height: 8),
                        // แสดงรายการตะกร้าที่เลือกพร้อมคลัง/ที่เก็บ
                        ...selectedCarts.map((cart) {
                          final isFirstCart = cart == selectedCarts.first;
                          final matchesFirst = cart.whcode == selectedCarts.first.whcode && cart.locationcode == selectedCarts.first.locationcode;
                ...
                onPressed: selectedCount >= 2
                    ? () {
                        ...
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => HandheldMergeScreen(
                              cart: selectedCarts,
                            ),
                          ),
                        ).then((value) {
          ...
          actions: <Widget>[
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                await _webServiceRepository.approveSaveHandHeld(cart).then((value) {
                  if (value.success) {
...
// Bottom Sheet สำหรับแสดงรายการตะกร้าย่อย
