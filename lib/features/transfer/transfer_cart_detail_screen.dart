...
import 'package:mobilestock/features/transfer/transfer_cart_item_search.dart';
...
class TransferCartDetailScreen extends StatefulWidget {
  final CartModel cart;
  final int ismerge;
  const TransferCartDetailScreen({super.key, required this.cart, required this.ismerge});

  @override
  State<TransferCartDetailScreen> createState() => _TransferCartDetailScreenState();
}

class _TransferCartDetailScreenState extends State<TransferCartDetailScreen> {
                      ...
                      _buildActionButton(
                        ...
                        onTap: () async {
                          final res = await Navigator.push(
                            context,
                            MaterialPageRoute(builder: (context) => const TransferCartItemSearch()),
                          );
                          if (res != null) {
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            if (widget.ismerge == 0)
              GestureDetector(
                onTap: () => widget.cart.status != 5 ? _showEditQuantityDialog(index) : null,
                child: Container(
