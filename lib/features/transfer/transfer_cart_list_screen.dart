...
import 'package:mobilestock/features/transfer/transfer_cart_detail_screen.dart';
import 'package:mobilestock/features/transfer/transfer_cart_form_screen.dart';
import 'package:mobilestock/features/transfer/transfer_cart_merge_screen.dart';
import 'package:mobilestock/features/transfer/transfer_cart_read_detail_screen.dart';
import 'package:mobilestock/model/cart_model.dart';
import 'package:mobilestock/repository/webservice_repository.dart';

class TransferCartListScreen extends StatefulWidget {
  const TransferCartListScreen({super.key});

  @override
  State<TransferCartListScreen> createState() => _TransferCartListScreenState();
}

class _TransferCartListScreenState extends State<TransferCartListScreen> {
                    ...
                    child: _buildActionButton(
                      ...
                      onPressed: () {
                        final res = Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => TransferCartFormScreen(cart: CartModel(docno: '')),
                          ),
                        );
                        res.then((value) {
                          getCartList();
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            _searchQuery.isEmpty ? 'ยังไม่มีตะกร้าโอนสินค้า' : 'ไม่พบข้อมูลที่ค้นหา',
            style: TextStyle(
              fontSize: 16,
                          ...
                          _buildIconButton(
                            ...
                            onPressed: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => TransferCartFormScreen(cart: cart),
                                ),
                              ).then((value) {
                                getCartList();
                              });
                        ...
                        _buildIconButton(
                          ...
                          onPressed: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => TransferCartDetailScreen(cart: cart, ismerge: cart.ismerge),
                              ),
                            ).then((value) {
                              getCartList();
                            });
    ...
    await _webServiceRepository.getCartDetail(docno).then((value) {
      ...
      if (value.success) {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => TransferCartReadDetailScreen(
              cart: cart,
                ...
                onPressed: selectedCount >= 2
                    ? () {
                        ...
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => TransferCartMergeScreen(
                              cart: selectedCarts,
                            ),
                          ),
                        ).then((value) {
          ...
          actions: <Widget>[
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                await _webServiceRepository.SaveTransfer(cart).then((value) {
                  if (value.success) {
...
// Bottom Sheet สำหโอนแสดงรายการตะกร้าย่อย
