...
class TransferCartMergeScreen extends StatefulWidget {
  final List<CartModel> cart;
  const TransferCartMergeScreen({super.key, required this.cart});

  @override
  State<TransferCartMergeScreen> createState() => _TransferCartMergeScreenState();
}

class _TransferCartMergeScreenState extends State<TransferCartMergeScreen> {
  // Theme Colors
  ...
  void initState() {
    ...
    if (widget.cart.isNotEmpty) {
      ...
      // โหลดคลังปลายทางและที่เก็บปลายทางจาก cart แรก
      if (widget.cart[0].whto.isNotEmpty) {
        selectedWarehouseTo = WarehouseModel(code: widget.cart[0].whto, name: widget.cart[0].whtoname);
      }
      if (widget.cart[0].locationto.isNotEmpty) {
        selectedLocationTo = LocationModel(code: widget.cart[0].locationto, name: widget.cart[0].locationtoname);
      }
  ...
  void getLocationTo(String whcode, {bool resetSelection = true}) async {
    await _webServiceRepository.getLocation(whcode).then((value) {
      if (value.success) {
        setState(() {
          locationsTo = (value.data as List).map((data) => LocationModel.fromJson(data)).toList();
          // reset เฉพาะเมื่อเปลี่ยนคลังปลายทางใหม่
          if (resetSelection) {
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final isSelected = warehouse.code == selectedWarehouseTo.code;
                  return ListTile(
                    leading: Container(
                    ...
                    onTap: () {
                      setState(() {
                        selectedWarehouseTo = warehouse;
                        selectedLocationTo = LocationModel();
                        locationsTo = [];
                      });
                      getLocationTo(warehouse.code);
  ...
  void _showLocationToSelector() {
    if (selectedWarehouseTo.code.isEmpty) {
      _showSnackBar('กรุณาเลือกคลังปลายทางก่อน', isError: true);
      return;
    }
      ...
      builder: (context) => Container(
        ...
        child: Column(
          children: [
            ...
            Expanded(
              child: locationsTo.isEmpty
                  ? const Center(
                      child: Text(
                        'ไม่พบที่เก็บในคลังนี้',
                        style: TextStyle(color: Color(0xFF64748B)),
                      ),
                    )
                  : ListView.builder(
                      itemCount: locationsTo.length,
                      itemBuilder: (context, index) {
                        final location = locationsTo[index];
                        final isSelected = location.code == selectedLocationTo.code;
                        return ListTile(
                          leading: Container(
                            padding: const EdgeInsets.all(8),
                            decoration: BoxDecoration(
                          ...
                          trailing: isSelected ? const Icon(Icons.check_circle, color: primaryBlue) : null,
                          onTap: () {
                            setState(() => selectedLocationTo = location);
                            Navigator.pop(context);
                          },
                        );
                      },
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    // คลังปลายทางและที่เก็บปลายทาง Card
                    _buildWarehouseLocationToCard(),
                    const SizedBox(height: 16),
  ...
  Widget _buildWarehouseLocationToCard() {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.move_down, color: warningOrange, size: 18),
              ),
              const SizedBox(width: 10),
              const Text(
                'คลังปลายทางและที่เก็บปลายทาง',
          ...
          // คลังปลายทาง (Read only - แสดงจาก cart ที่เลือก)
          _buildInfoField(
            label: 'คลังปลายทาง',
            value: selectedWarehouseTo.code.isNotEmpty ? '${selectedWarehouseTo.code} - ${selectedWarehouseTo.name}' : 'ไม่ระบุ',
            icon: Icons.warehouse_outlined,
          ),
          const SizedBox(height: 12),

          // ที่เก็บปลายทาง (Read only - แสดงจาก cart ที่เลือก)
          _buildInfoField(
            label: 'ที่เก็บปลายทาง',
            value: selectedLocationTo.code.isNotEmpty ? '${selectedLocationTo.code} - ${selectedLocationTo.name}' : 'ไม่ระบุ',
            icon: Icons.location_on_outlined,
          ),
        ],
      ),
  ...
  Widget _buildSelectableField({
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Icon(icon, color: isEmpty ? const Color(0xFF94A3B8) : primaryBlue, size: 20),
            const SizedBox(width: 10),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
            ...
            Icon(
              ...
              color: isEmpty ? const Color(0xFFCBD5E1) : primaryBlue,
              size: 20,
            ),
          ],
        ),
  ...
  Future<void> _mergeCart() async {
    ...
    if (selectedWarehouseTo.code.isEmpty || selectedLocationTo.code.isEmpty) {
      _showSnackBar('กรุณาเลือกคลังปลายทางและที่เก็บปลายทาง', isError: true);
      return;
    }
