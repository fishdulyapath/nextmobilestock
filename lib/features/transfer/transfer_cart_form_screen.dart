...
class TransferCartFormScreen extends StatefulWidget {
  final CartModel cart;
  const TransferCartFormScreen({super.key, required this.cart});

  @override
  State<TransferCartFormScreen> createState() => _TransferCartFormScreenState();
}

class _TransferCartFormScreenState extends State<TransferCartFormScreen> {
  ...
  // คลังและที่เก็บปลายทาง
  WarehouseModel selectedWarehouseTo = WarehouseModel();
  LocationModel selectedLocationTo = LocationModel();
  List<LocationModel> locationsTo = [];
  ...
  void initState() {
    ...
    if (widget.cart.docno.isNotEmpty) {
      ...
      // โหลดคลังและที่เก็บปลายทาง
      if (widget.cart.whto.isNotEmpty) {
        selectedWarehouseTo = WarehouseModel(code: widget.cart.whto, name: widget.cart.whtoname);
      }
      if (widget.cart.locationto.isNotEmpty) {
        selectedLocationTo = LocationModel(code: widget.cart.locationto, name: widget.cart.locationtoname);
      }
  ...
  // โหลดที่เก็บปลายทาง
  void getLocationTo() async {
    await _webServiceRepository.getLocation(selectedWarehouseTo.code).then((value) {
      if (value.success) {
        setState(() {
          locationsTo = (value.data as List).map((data) => LocationModel.fromJson(data)).toList();
          if (widget.cart.docno.isNotEmpty && widget.cart.locationto.isNotEmpty) {
            // หาชื่อที่เก็บจาก list
            final found = locationsTo.firstWhere(
              (loc) => loc.code == widget.cart.locationto,
              orElse: () => LocationModel(code: widget.cart.locationto, name: ''),
            );
            selectedLocationTo = found;
          }
        });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text(
          isEdit ? 'แก้ไขตะกร้า' : 'สร้างตะกร้าใหม่',
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 20), // ที่เก็บ
                    ...
                    // คลังปลายทาง
                    _buildSectionTitle('คลังปลายทาง', Icons.warehouse_outlined),
                    const SizedBox(height: 8),
                    _buildSelectorField(
                      label: 'คลังปลายทาง',
                      value: selectedWarehouseTo.code.isNotEmpty ? "${selectedWarehouseTo.code} - ${selectedWarehouseTo.name}" : null,
                      hint: 'เลือกคลังปลายทาง (ถ้ามี)',
                      icon: Icons.warehouse_outlined,
                      onTap: _showWarehouseToSelector,
                    ),

                    const SizedBox(height: 20),

                    // ที่เก็บปลายทาง
                    _buildSectionTitle('ที่เก็บปลายทาง', Icons.location_on_outlined),
                    const SizedBox(height: 8),
                    _buildSelectorField(
                      label: 'ที่เก็บปลายทาง',
                      value: selectedLocationTo.code.isNotEmpty ? "${selectedLocationTo.code} - ${selectedLocationTo.name}" : null,
                      hint: selectedWarehouseTo.code.isEmpty
                          ? "กรุณาเลือกคลังปลายทางก่อน"
                          : locationsTo.isEmpty
                              ? "กำลังโหลด..."
                              : "เลือกที่เก็บปลายทาง (ถ้ามี)",
                      icon: Icons.location_on_outlined,
                      onTap: _showLocationToSelector,
                      enabled: selectedWarehouseTo.code.isNotEmpty,
                    ),
                    const SizedBox(height: 20),
  ...
  // Selector สำหรับคลังปลายทาง
  void _showWarehouseToSelector() {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) => _WarehouseLocationSelector<WarehouseModel>(
        ...
        icon: Icons.warehouse_outlined,
        items: warehouses,
        selectedItem: selectedWarehouseTo.code.isNotEmpty ? selectedWarehouseTo : null,
        itemBuilder: (item) => _SelectorItemData(
          code: item.code,
          name: item.name,
          icon: Icons.warehouse_outlined,
        ...
        onSelected: (item) {
          setState(() {
            selectedWarehouseTo = item;
            locationsTo = [];
            selectedLocationTo = LocationModel();
          });
          Future.delayed(const Duration(milliseconds: 300), () {
            getLocationTo();
          });
  ...
  // Selector สำหรับที่เก็บปลายทาง
  void _showLocationToSelector() {
    if (selectedWarehouseTo.code.isEmpty || locationsTo.isEmpty) return;
    ...
    showModalBottomSheet(
      ...
      backgroundColor: Colors.transparent,
      builder: (context) => _WarehouseLocationSelector<LocationModel>(
        title: 'เลือกที่เก็บปลายทาง',
        icon: Icons.location_on_outlined,
        items: locationsTo,
        selectedItem: selectedLocationTo.code.isNotEmpty ? selectedLocationTo : null,
        itemBuilder: (item) => _SelectorItemData(
          code: item.code,
          name: item.name,
          icon: Icons.location_on_outlined,
  ...
  Future<void> _saveCart() async {
    ...
    // ตรวจสอบคลังปลายทางและที่เก็บปลายทาง (บังคับ)
    if (selectedWarehouseTo.code.isEmpty || selectedLocationTo.code.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Row(
            children: [
              Icon(Icons.warning_amber_rounded, color: Colors.white),
              SizedBox(width: 10),
              Expanded(child: Text("กรุณาเลือกคลังปลายทางและที่เก็บปลายทาง")),
            ],
          ),
          backgroundColor: const Color(0xFFF59E0B),
          behavior: SnackBarBehavior.floating,
    ...
    try {
      ...
      if (widget.cart.docno.isEmpty) {
        ...
        final value = await _webServiceRepository.createCart(
          ...
          '',
          transflag,
          whto: selectedWarehouseTo.code,
          locationto: selectedLocationTo.code,
        );
  ...
  String generateBasketNumber() {
    ...
    return 'TF$year$month$day$hour$minute$randomDigits';
  }
}
